class MainCasesController < ApplicationController
  ...
  def create_case_memo
    redirect_to case_memos_main_case_url(@main_case), alert: '您的没有在该案件创建便签的权限！' and return if not (@main_case.temp_ident_user?(@current_user) || @main_case.wtr?(@current_user) || @current_user.center_department_director_user? || @current_user.center_admin_user?)
