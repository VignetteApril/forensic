...
class ApplicationController < ActionController::Base
  ...
  # 用户登录验证
  def authorize
    # if get_license_type() != 'application' and get_license_type() != 'platform'
    # if get_license_type() == 'blank'
    #   # 没有授权码的情况
    #   redirect_to "/s_license/codes/new", notice: "您初次使用系统，请先输入授权码。"
    # elsif get_license_type() == 'error' or get_license_type() == ''
    #   # 授权码错误的情况
    #   redirect_to "/s_license/codes/new", notice: "对不起，您授权文件格式不正确，如果需要使用，请购买授权。"
    # elsif get_license_type() == 'application'
    #   # 授权类型是桌面应用的情况
    ...
    #       if u = User.find_by_api_key(params[:apikey])
    #         @current_user = u
    #         # session['cas']['user'] = u.login
    #         session['cas'] = { user: u.login }
    #         return
    #       end
    #     end
    ...
    #       return
    #     end
    #     @current_user = User.find_by_login(session['cas']['user'])
